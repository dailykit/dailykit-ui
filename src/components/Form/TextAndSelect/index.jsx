...
const TextAndSelect = ({ label, name, value, hasError, options, onChange }) => {
    ...
    return (
        <TextAndSelectStyle>
            <Text label={ label } name={ field.name } value={ field.value } hasError={ hasError } onChange={ handleTextChange }/>
            <DropDown>
                ...
                <div hidden={ !isOptionsVisible }>
                    {
                        options.map(option => {
                            ...
                            return <div key={ option.id } onClick={ () => handleOptionChange(option) }> { option.title } </div> 
                        })
                    }
                </div>
            </DropDown>
